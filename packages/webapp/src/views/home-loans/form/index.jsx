...
import { saveLabel } from "@/services/label";
import { LABEL_COLORS } from "@/constants/app.constant";
...
const colorOptions = colorKeys.map((color) => {
	return { value: color, label: color, color: LABEL_COLORS[color].dot };
});
...
const LabelForm = (props) => {
	...
	useEffect(() => {
		if (isEdit && data) {
			...
			values.color = _.find(colorOptions, (row) => row.value === data.color);
	...
	const onSubmit = async ({ values }) => {
		try {
			...
			setLoading(true);
			const response = await saveLabel(values);
			setLoading(false);
			toast.push(
